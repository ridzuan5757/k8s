...
INVENTORY = os.getenv("INVENTORY") if os.getenv(
    "INVENTORY") is not None else "localhost"
PAYMENT = os.getenv("PAYMENT") if os.getenv(
    "PAYMENT") is not None else "localhost"
AUTH = os.getenv("AUTH") if os.getenv("AUTH") is not None else "localhost"
SLEEP_TIME_IN_SECONDS = os.getenv("SLEEP_TIME_IN_SECONDS") if os.getenv(
    "SLEEP_TIME_IN_SECONDS") is not None else 1

DB_NAME = 'APM'
HOST = os.getenv("MYSQL_HOST") if os.getenv(
    "MYSQL_HOST") is not None else "localhost"
PORT = int(os.getenv("MYSQL_PORT")) if os.getenv(
    "MYSQL_PORT") is not None else 3306
...
otlp_exporter = OTLPSpanExporter(
    endpoint="{}:4317".format(OTLP), insecure=True)
tracerProvider.add_span_processor(
...
def getDBCnx():
    cnx = mysql.connector.connect(
        user="root", host=HOST, port=PORT, database=DB_NAME)
    return cnx
    ...
    try:
        with tracer.start_as_current_span("client_checkout") as checkout_trace_group:
            trace_id = get_hexadecimal_trace_id(
                checkout_trace_group.get_span_context().trace_id)
...
def createOrder():
    ...
    try:
        with tracer.start_as_current_span("client_create_order") as create_order_trace_group:
            trace_id = get_hexadecimal_trace_id(
                create_order_trace_group.get_span_context().trace_id)
...
def cancelOrder():
    ...
    try:
        with tracer.start_as_current_span("client_cancel_order") as cancel_order_trace_group:
            trace_id = get_hexadecimal_trace_id(
                cancel_order_trace_group.get_span_context().trace_id)
            cancelOrderAPIRequest = delete(
                "http://{}:8088/clear_order".format(ORDER))
...
def deliveryStatus():
    ...
    try:
        with tracer.start_as_current_span("client_delivery_status") as delivery_status_trace_group:
            trace_id = get_hexadecimal_trace_id(
                delivery_status_trace_group.get_span_context().trace_id)
...
def payOrder():
    ...
    try:
        with tracer.start_as_current_span("client_pay_order") as pay_order_trace_group:
            trace_id = get_hexadecimal_trace_id(
                pay_order_trace_group.get_span_context().trace_id)
