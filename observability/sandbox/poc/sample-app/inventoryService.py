...
DATABASE = os.getenv("DATABASE") if os.getenv(
    "DATABASE") is not None else "localhost"
...
def read_inventory():
    ...
    if errorRate < READ_ERROR_RATE_THRESHOLD:
        ...
        raise Error('Read Inventory Failed - Service Unavailable',
                    status_code=503)
    else:
...
def update_inventory():
    ...
    if errorRate < UPDATE_ERROR_RATE_THRESHOLD:
        ...
        raise Error('Update Inventory Failed - Service Unavailable',
                    status_code=503)
    else:
...
def delete_inventory():
    ...
    if errorRate < DELETE_ERROR_RATE_THRESHOLD:
        logs('Inventory', 'Update operation failed - Service Unavailable: 503')
        raise Error('Delete Inventory Failed - Service Unavailable',
                    status_code=503)
    else:
