...
def recommend():
    ...
    if errorRate < RECOMMEND_ERROR_RATE_THRESHOLD:
        ...
        raise Error(
            'Recommendation Retrieval Failed; TooManyRequests', status_code=429)
    else:
        with tracer.start_as_current_span("recommend"):
            ...
            availItems = [(itemId, count) for itemId,
                          count in readInventoryResponse.json().items() if count > 0]

            if num is not None:
                logs('Recommendation',
                     'Successfully returning specified number of items to customer')
                return jsonify(
                    {
